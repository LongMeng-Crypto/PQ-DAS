...
pub const V3_OPENED_CELLS: usize = 19;
...
impl Relation {
    /// Returns which V3 relation blocks are enabled in the LeanVM guest.
...
/// Maps a V3 physical even-first codeword index back to the logical FFT-domain index.
...
/// Encodes data and constructs V3's row digests and column-root commitment.
...
/// Returns the canonical byte size of the public V3-base commitment.
...
/// Reconstructs the original blobs after verifying enough distinct V3 cell columns.
...
/// Computes the V3 subset-soundness log2 failure bound without replacement.
...
fn guest_source(relation: Relation) -> ProgramSource {
    let source = match relation {
        Relation::Full => include_str!("../../zkdsl/v3_base/full.py"),
...
fn compilation_flags(commitment: &Commitment, relation: Relation) -> Result<CompilationFlags, DemoError> {
    ...
    let _row_hashes_ptr = read_only_cursor;
...
/// Recomputes V3 Fiat-Shamir data, generates physical-order L, and compiles the full V3 guest.
...
/// Recomputes V3 Fiat-Shamir data and compiles the selected relation benchmark guest.
...
/// Proves the V3 cell-first commitment and RS dot-product statement.
...
/// Verifies a LeanVM proof against an already rebuilt V3 statement.
...
/// Rebuilds the verifier's full V3 statement and verifies the LeanVM proof.
...
/// Rebuilds the verifier's selected V3 relation statement and verifies the LeanVM proof.
...
/// Rebuilds the public V3 statement, verifies its proof, and checks openings.
