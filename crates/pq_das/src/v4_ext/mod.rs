...
/// Computes log2 of the V4 subset-soundness bound for sampling with replacement.
...
/// Encodes extension-field data and constructs V4's column-root-only commitment.
...
fn guest_source() -> ProgramSource {
    ProgramSource::Raw(include_str!("../../zkdsl/v4_ext/full.py").to_string())
}
...
/// Recomputes Fiat-Shamir, generates physical-order L, and compiles the V4-ext guest.
...
/// Proves V4-ext's column-root commitment and extension-field RS dot-product statement.
...
/// Verifies a LeanVM proof against an already rebuilt V4-ext statement.
...
/// Verifies opened extension-field cells against the public outer column root.
...
/// Reconstructs extension-field blobs after verifying enough distinct V4-ext cell columns.
